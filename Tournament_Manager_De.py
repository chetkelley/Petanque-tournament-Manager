...
class Database:
    ...
    def add_history(self, conn, match_id: int, terrain: int,
                    team_a: str, team_b: str, score_a: int, score_b: int, round_num: int):
        conn.execute(
    ...
    def get_payments(self) -> list:
        """Return each registered player with total paid and balance owed."""
        with self.connect() as conn:
            fee = self.get_fee()
...
class PetanqueProMaster:
    ...
    def _build_tab1(self):
        ...
        tk.Label(row2, text="Bahnen:", font=("Arial", 10)).pack(side="left", padx=(5, 2))
        ...
        tk.Label(row2, text="Anzahl Runden:", font=("Arial", 10)).pack(side="left", padx=(0, 2))
        self.num_rounds_entry = tk.Entry(row2, width=3)
        self.num_rounds_entry.pack(side="left", padx=(0, 10))

        row3 = tk.Frame(mgmt)
        row3.pack(fill="x", pady=(10, 0))
        tk.Label(row3, text="📢 DURCHSAGE:", font=("Arial", 10, "bold")).pack(side="left", padx=5)
    ...
    def _build_tab3(self):
        ...
        fee_frame = tk.LabelFrame(self.tab3, text="Startgeld-Einstellung", padx=10, pady=10)
        fee_frame.pack(fill="x", pady=5)

        tk.Label(fee_frame, text="Startgeld pro Team/Spieler (€):",
        ...
        tk.Button(fee_frame, text="Betrag speichern", command=self._save_fee,
                  bg="#27ae60").pack(side="left", padx=10)

        # Summary bar
        self.lbl_summary = tk.Label(self.tab3, text="", font=("Arial", 11, "bold"),
                                    bg="#2c3e50", fg="white", pady=6)
        self.lbl_summary.pack(fill="x", pady=(5, 0))

        # Payment entry row
        pay_frame = tk.LabelFrame(self.tab3, text="Zahlung erfassen", padx=10, pady=10)
        pay_frame.pack(fill="x", pady=5)

        tk.Label(pay_frame, text="Betrag (€):", font=("Arial", 11)).pack(side="left", padx=5)
        self.entry_payment = tk.Entry(pay_frame, width=8, font=("Arial", 12), justify="center")
        self.entry_payment.pack(side="left", padx=5)
        tk.Button(pay_frame, text="Zahlung buchen",
                  command=self._record_payment, bg="#3498db").pack(side="left", padx=10)
        tk.Label(pay_frame, text="(Team/Spieler in der Liste auswählen, dann hier buchen)",
                 fg="gray", font=("Arial", 9)).pack(side="left", padx=5)

        # Payment list
        cols = ("name", "paid", "owed", "status")
        self.pay_tree = ttk.Treeview(self.tab3, columns=cols, show="headings")
        for col, head, width in [
            ("name",   "Team / Spieler", 250),
            ("paid",   "Bezahlt (€)",    130),
            ("owed",   "Offen (€)",      130),
            ("status", "Status",         120),
        ]:
        ...
        # Colour tag for fully paid rows
    ...
    def _refresh_payments(self):
        ...
        for p in self.db.get_payments():
            if p["owed"] <= 0:
                tag, status = "paid",    "✅ Bezahlt"
            elif p["paid"] > 0:
                tag, status = "partial", "⚠️ Teilzahlung"
            else:
                tag, status = "unpaid",  "❌ Ausstehend"
            self.pay_tree.insert("", "end", tags=(tag,),
                values=(p["name"], f"{p['paid']:.2f}", f"{p['owed']:.2f}", status))

        s = self.db.get_payment_summary()
        self.lbl_summary.config(
            text=f"  Fällig: €{s['total_due']:.2f}   |   "
                 f"Eingenommen: €{s['collected']:.2f}   |   "
                 f"Ausstehend: €{s['outstanding']:.2f}   |   "
                 f"Vollständig bezahlt: {s['fully_paid']} / {s['n']}"
        )
    ...
    def _save_fee(self):
        try:
            ...
            messagebox.showerror("Fehler", "Bitte einen gültigen Betrag eingeben (z.B. 15.00)")

    def _record_payment(self):
        sel = self.pay_tree.selection()
        if not sel:
            messagebox.showwarning("Auswahl", "Bitte zuerst ein Team/Spieler in der Liste auswählen.")
            return
        ...
        try:
            ...
            messagebox.showerror("Fehler", "Bitte einen gültigen Betrag eingeben (z.B. 5.00)")
            return

        # Warn if payment would exceed the fee
        fee = self.db.get_fee()
        ...
        if fee > 0 and already_paid + amount > fee:
            if not messagebox.askyesno("Überzahlung",
                    f"{name} hätte dann mehr bezahlt als das Startgeld (€{fee:.2f}). Trotzdem buchen?"):
                return
    ...
    def _assign_lanes(self, conn, pairs: list[tuple[str, str]], round_num: int):
